...
export const obtenerApiKeyDisponible = (): string => {
  ...
  if (!keyDisponible) {
    ...
    throw new Error("❌ CRÍTICO: No hay API keys disponibles. Revisa ScraperAPI dashboard.");
  }
...
export const rotarKey = (keyActual: string, error: boolean = false): string => {
  ...
  if (error && indexActual !== -1) {
    ...
    if (keys[indexActual].erroresConsecutivos >= 5) {
      ...
      console.error(`🚫 Key desactivada por errores: ${keyActual.slice(0, 8)}...`);
    }
...
export const validarCreditosReales = async (apiKey: string): Promise<number> => {
  try {
    // ScraperAPI tiene un endpoint para ver cuenta
    const response = await axios.get(`https://api.scraperapi.com/account?api_key=${apiKey}`);
...
export const resetearScraperKeys = async (): Promise<void> => {
  ...
  for (const key of keys) {
    ...
    console.log(`✅ Key ${key.key.slice(0, 8)}... reseteada. Créditos reales: ${creditosReales}`);

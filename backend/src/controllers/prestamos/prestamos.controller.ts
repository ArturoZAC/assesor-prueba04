...
export async function recalcularDetracciones(
  req: Request,
  res: Response
): Promise<any> {
  try {
    ...
    const nuevoValor = calcularDetraccion(
      Number(prestamo.potencial),
      prestamo.tipo
    );
...
export async function registrarPrestamo(
  ...
  const interes =
    (((Number(capital_dolares ?? 0) + Number(capital_soles ?? 0)) * tasa) /
      100 /
      30) *
    dias;
  ...
  const cobroTotal = parseFloat(
    (capital_dolares ?? 0) + (capital_soles ?? 0) + interes
  );
...
export async function obtenerPrestamos(req: any, res: any) {
  ...
  try {
    ...
    if (estadoParam) {
      const estadoValido = Object.values(EstatusPrestamo).includes(
        estadoParam as EstatusPrestamo
      );
      if (estadoValido) {
...
export async function editarPrestamo(
  ...
  const interes =
    (((Number(capital_dolares) + Number(capital_soles)) * Number(tasa)) /
      100 /
      30) *
    Number(dias);
  const potencial =
    Number(interes) * Number(moneda === "USD" ? Number(tc ?? 0) : 1);
  const rendimiento = Number(potencial) / 1.18;
  const resultadoIGV = Number(rendimiento) * 0.18;
  const cobroTotal = parseFloat(
    String(
      Number(capital_dolares ?? 0) +
        Number(capital_soles ?? 0) +
        Number(interes)
    )
  );
  const detraccion = calcularDetraccion(potencial, tipo);
      ...
      const prestamo = await prisma.prestamoOperacion.update({
        ...
        data: {
          numero_prestamo: numero_prestamo
            ? numero_prestamo
            : prestamoBuscado.numero_prestamo,
          igv: resultadoIGV,
          tasa: tasa,
          devolucion: new Date(devolucion),
          fechaInicial: new Date(fechaInicial),
          ...
          estatus: estatus,
          ganancia:
            String(estatus).toUpperCase() === "PAGADO" ? rendimiento : 0,
          factura: "",
        },
      });
...
export const obtenerTotalCapitalPorMes = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const formattedData = allMonths.map((month) => {
      ...
      return {
        month: monthName,
        soles: dataForMonth?.totalSoles
          ? parseFloat(dataForMonth.totalSoles.toFixed(2))
          : 0,
        dolares: dataForMonth?.totalDolares
          ? parseFloat(dataForMonth.totalDolares.toFixed(2))
          : 0,
      };
    });
    ...
    const finalData = allMonths.map((monthNumber) => {
      ...
      const existingMonthData = formattedData.find(
        (m) => m.month === monthName
      );
    ...
    res
      .status(500)
      .json({ error: "Error al obtener el total del capital por mes" });
  }
  /*
  try {
...
export async function getTotal(
  ...
  try {
    ...
    let accumulatedSumaTcNoCero = 0;
    let accumulatedCantidadTcNoCero = 0;
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      //     if (tc !== 0) {
      //       cantidadDeValorHayados++;
      //       sumaTotalDeTc += tc;
      //     }
      //   });

      //   console.log({ cantidadDeValorHayados });
      //   console.log({ sumaTotalDeTc });

      //   const promedio = cantidadDeValorHayados > 0 ? sumaTotalDeTc / cantidadDeValorHayados : 0;
      ...
      const promedioTc =
        cantidadTcNoCero > 0 ? sumaTcNoCero / cantidadTcNoCero : 0;
      ...
      if (cantidadTcNoCero > 0) {
        ...
        accumulatedCantidadTcNoCero += cantidadTcNoCero;
      }
    ...
    //*Para resolver el problema de tc 23-11
    const accumulatedTotalTc =
      accumulatedCantidadTcNoCero > 0
        ? accumulatedSumaTcNoCero / accumulatedCantidadTcNoCero
        : 0;
    //*
    const accumulatedPromedioDias =
      totalPrestamosAccumulated > 0
        ? totalDaysAccumulated / totalPrestamosAccumulated
        : null;
    const accumulatedPromedioTasa =
      totalPrestamosAccumulated > 0
        ? totalRateAccumulated / totalPrestamosAccumulated
        : null;
    ...
    const accumulatedObject = {
      ...
      promedioDias:
        accumulatedPromedioDias !== null
          ? parseFloat(accumulatedPromedioDias.toFixed(2))
          : null,
      promedioTasa:
        accumulatedPromedioTasa !== null
          ? parseFloat(accumulatedPromedioTasa.toFixed(4))
          : null,
    };
    ...
    console.error(
      `Error fetching monthly totals for year ${year} with accumulated:`,
      error
    );
    res.status(500).json({
...
export async function getTotalPrestamosFLUJO(
  yearParam: string
): Promise<any | undefined> {
  let year: number;
...
export async function obtenerPrestamosAnulados(req: any, res: any) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Error al obtener prestamos anulados" });
  } finally {
    await prisma.$disconnect();
  }
}
